...
for hue in hues:
    temp_df_cd = cd[cd['abundance_cutoff_2'] == hue][snakemake.wildcards.numerical_features]
    df_list_cd.append(temp_df_cd)
    temp_df_haca = haca[haca['abundance_cutoff_2'] == hue][snakemake.wildcards.numerical_features]
    df_list_haca.append(temp_df_haca)
    color = color_dict[hue]
    colors.append(color)


ft.density_x(df_list_cd, snakemake.wildcards.numerical_features, 'Density', 'linear', '',
        colors, hues, snakemake.output.density_features_cd)

ft.density_x(df_list_haca, snakemake.wildcards.numerical_features, 'Density', 'linear', '',

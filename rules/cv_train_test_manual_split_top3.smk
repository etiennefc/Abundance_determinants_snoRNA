...
### Then train only on the top 3 features (to see if the performance is high
### enough to apply this model to new species): box_consensus_score, sno_mfe and terminal_stem_mfe

rule fill_na_feature_scaling_after_manual_split_top3:
...
rule hyperparameter_tuning_cv_scale_after_manual_split_top3:
    ...
    input:
        X_cv = rules.fill_na_feature_scaling_after_manual_split_top3.output.cv,
        y_cv = rules.fill_na_feature_scaling_after_manual_split_top3.output.y_cv
    output:
        best_hyperparameters = os.path.join(config['path']['hyperparameter_tuning'],
                                    '{models2}_best_params_scale_after_split_top3_{manual_iteration}.tsv')
    params:
...
rule train_models_scale_after_manual_split_top3:
    ...
    input:
        X_train = rules.fill_na_feature_scaling_after_manual_split_top3.output.train,
        y_train = rules.fill_na_feature_scaling_after_manual_split_top3.output.y_train,
        best_hyperparameters = rules.hyperparameter_tuning_cv_scale_after_manual_split_top3.output.best_hyperparameters
    output:
        pickled_trained_model = os.path.join(config['path']['trained_models'],
                                    '{models2}_trained_scale_after_split_top3_{manual_iteration}.sav'),
        training_accuracy = os.path.join(config['path']['training_accuracy'],
                                    '{models2}_training_accuracy_scale_after_split_top3_{manual_iteration}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/train_models_scale_after_split.py"

rule test_models_scale_after_manual_split_top3:
    """ Test model performance on unseen test data and return their accuracies."""
    input:
        X_test = rules.fill_na_feature_scaling_after_manual_split_top3.output.test,
        y_test = rules.fill_na_feature_scaling_after_manual_split_top3.output.y_test,
        pickled_trained_model = rules.train_models_scale_after_manual_split_top3.output.pickled_trained_model
    output:
        test_accuracy = os.path.join(config['path']['test_accuracy'],
                                    '{models2}_test_accuracy_scale_after_split_top3_{manual_iteration}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/test_models_scale_after_split.py"

rule confusion_matrix_f1_scale_after_manual_split_top3:
    ...
    output:
        confusion_matrix = os.path.join(config['path']['confusion_matrix_f1'],
                        '{models2}_confusion_matrix_w_f1_score_scale_after_split_top3_{manual_iteration}.tsv'),
        info_df = os.path.join(config['path']['confusion_matrix_f1'],
                        '{models2}_confusion_matrix_values_per_sno_scale_after_split_top3_{manual_iteration}.tsv')
    conda:
        "../envs/python.yaml"
    script:

...
rule fill_na_feature_scaling_after_split_10_iterations_only_hamming:
    ...
    output:
        cv = os.path.join(config['path']['scaled_feature_10_iterations'],
                        'all_features_labels_scaled_cv_only_hamming_{iteration}.tsv'),
        train = os.path.join(config['path']['scaled_feature_10_iterations'],
                        'all_features_labels_scaled_train_only_hamming_{iteration}.tsv'),
        test = os.path.join(config['path']['scaled_feature_10_iterations'],
                        'all_features_labels_scaled_test_only_hamming_{iteration}.tsv'),
        y_cv = os.path.join(config['path']['scaled_feature_10_iterations'],
                        'labels_cv_only_hamming_{iteration}.tsv'),
        y_train = os.path.join(config['path']['scaled_feature_10_iterations'],
                        'labels_train_only_hamming_{iteration}.tsv'),
        y_test = os.path.join(config['path']['scaled_feature_10_iterations'],
                        'labels_test_only_hamming_{iteration}.tsv')
    params:
        random_state = config['random_state_iteration']
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/scale_features_after_split_10_iterations_only_hamming.py"

rule hyperparameter_tuning_cv_scale_after_split_10_iterations_only_hamming:
    ...
    input:
        X_cv = rules.fill_na_feature_scaling_after_split_10_iterations_only_hamming.output.cv,
        y_cv = rules.fill_na_feature_scaling_after_split_10_iterations_only_hamming.output.y_cv
    output:
        best_hyperparameters = os.path.join(config['path']['hyperparameter_tuning'],
                                    '{models2}_best_params_scale_after_split_only_hamming_{iteration}.tsv')
    params:
...
rule train_models_scale_after_split_10_iterations_only_hamming:
    ...
    input:
        X_train = rules.fill_na_feature_scaling_after_split_10_iterations_only_hamming.output.train,
        y_train = rules.fill_na_feature_scaling_after_split_10_iterations_only_hamming.output.y_train,
        best_hyperparameters = rules.hyperparameter_tuning_cv_scale_after_split_10_iterations_only_hamming.output.best_hyperparameters
    output:
        pickled_trained_model = os.path.join(config['path']['trained_models'],
                                    '{models2}_trained_scale_after_split_only_hamming_{iteration}.sav'),
        training_accuracy = os.path.join(config['path']['training_accuracy'],
                                    '{models2}_training_accuracy_scale_after_split_only_hamming_{iteration}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/train_models_scale_after_split.py"

rule test_models_scale_after_split_10_iterations_only_hamming:
    """ Test model performance on unseen test data and return their accuracies."""
    input:
        X_test = rules.fill_na_feature_scaling_after_split_10_iterations_only_hamming.output.test,
        y_test = rules.fill_na_feature_scaling_after_split_10_iterations_only_hamming.output.y_test,
        pickled_trained_model = rules.train_models_scale_after_split_10_iterations_only_hamming.output.pickled_trained_model
    output:
        test_accuracy = os.path.join(config['path']['test_accuracy'],
                                    '{models2}_test_accuracy_scale_after_split_only_hamming_{iteration}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/test_models_scale_after_split.py"

rule confusion_matrix_f1_scale_after_split_10_iterations_only_hamming:
    ...
    output:
        confusion_matrix = os.path.join(config['path']['confusion_matrix_f1'],
                        '{models2}_confusion_matrix_w_f1_score_scale_after_split_only_hamming_{iteration}.tsv'),
        info_df = os.path.join(config['path']['confusion_matrix_f1'],
                        '{models2}_confusion_matrix_values_per_sno_scale_after_split_only_hamming_{iteration}.tsv')
    conda:
        "../envs/python.yaml"
    script:

...
# Do this cv-train-test 20 times (iterations) to see if models are comparable depending on the initial split of the 3 datasets

rule fill_na_feature_scaling_after_split_20_iterations:
    """ Fill NA values across columns in the feature df, fixing them at -5
        ...
        standard deviation of all values((x - mean)/std). Do this to create 20
        subsets of cv-train-test datasets."""
    input:
        feature_df = config['path']['one_hot_encoded_df_before_split']
    output:
        cv = os.path.join(config['path']['scaled_feature_20_iterations'],
                        'all_features_labels_scaled_cv_{iteration_20}.tsv'),
        train = os.path.join(config['path']['scaled_feature_20_iterations'],
                        'all_features_labels_scaled_train_{iteration_20}.tsv'),
        test = os.path.join(config['path']['scaled_feature_20_iterations'],
                        'all_features_labels_scaled_test_{iteration_20}.tsv'),
        y_cv = os.path.join(config['path']['scaled_feature_20_iterations'],
                        'labels_cv_{iteration_20}.tsv'),
        y_train = os.path.join(config['path']['scaled_feature_20_iterations'],
                        'labels_train_{iteration_20}.tsv'),
        y_test = os.path.join(config['path']['scaled_feature_20_iterations'],
                        'labels_test_{iteration_20}.tsv')
    params:
        random_state = config['random_state_iteration_20']
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/scale_features_after_split_20_iterations.py"

rule hyperparameter_tuning_cv_scale_after_split_20_iterations:
    ...
    input:
        X_cv = rules.fill_na_feature_scaling_after_split_20_iterations.output.cv,
        y_cv = rules.fill_na_feature_scaling_after_split_20_iterations.output.y_cv
    output:
        best_hyperparameters = os.path.join(config['path']['hyperparameter_tuning'],
                                    '{models2}_best_params_scale_after_split_{iteration_20}.tsv')
    params:
...
rule train_models_scale_after_split_20_iterations:
    ...
    input:
        X_train = rules.fill_na_feature_scaling_after_split_20_iterations.output.train,
        y_train = rules.fill_na_feature_scaling_after_split_20_iterations.output.y_train,
        best_hyperparameters = rules.hyperparameter_tuning_cv_scale_after_split_20_iterations.output.best_hyperparameters
    output:
        pickled_trained_model = os.path.join(config['path']['trained_models'],
                                    '{models2}_trained_scale_after_split_{iteration_20}.sav'),
        training_accuracy = os.path.join(config['path']['training_accuracy'],
                                    '{models2}_training_accuracy_scale_after_split_{iteration_20}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/train_models_scale_after_split.py"

rule test_models_scale_after_split_20_iterations:
    """ Test model performance on unseen test data and return their accuracies."""
    input:
        X_test = rules.fill_na_feature_scaling_after_split_20_iterations.output.test,
        y_test = rules.fill_na_feature_scaling_after_split_20_iterations.output.y_test,
        pickled_trained_model = os.path.join(config['path']['trained_models'],
                                    '{models2}_trained_scale_after_split_{iteration_20}.sav')
    output:
...
rule confusion_matrix_f1_scale_after_split_20_iterations:
    ...
    output:
        ...
        info_df = os.path.join(config['path']['confusion_matrix_f1'],
                        '{models2}_confusion_matrix_values_per_sno_scale_after_split_{iteration_20}.tsv')
    conda:
        "../envs/python.yaml"
    script:

...
### parameters of the models. Train only on TOP3 features (box_consensus_score,
### sno_mfe and terminal_stem_mfe)

rule fill_na_feature_scale_species_prediction_top3_rs:
    ...
    output:
        cv = 'results/tables/all_features_labels_scaled_cv_top3_species_prediction_{rs}.tsv',
        train = 'results/tables/all_features_labels_scaled_train_top3_species_prediction_{rs}.tsv',
        y_cv = 'results/tables/labels_cv_top3_species_prediction_{rs}.tsv',
        y_train = 'results/tables/labels_train_top3_species_prediction_{rs}.tsv'
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/scale_features_species_prediction_top3_rs.py"

rule hyperparameter_tuning_cv_species_prediction_top3_rs:
    ...
    input:
        X_cv = rules.fill_na_feature_scale_species_prediction_top3_rs.output.cv,
        y_cv = rules.fill_na_feature_scale_species_prediction_top3_rs.output.y_cv
    output:
...
rule train_models_species_prediction_top3_rs:
    ...
    input:
        X_train = rules.fill_na_feature_scale_species_prediction_top3_rs.output.train,
        y_train = rules.fill_na_feature_scale_species_prediction_top3_rs.output.y_train,
        best_hyperparameters = rules.hyperparameter_tuning_cv_species_prediction_top3_rs.output.best_hyperparameters
    output:
        pickled_trained_model = os.path.join(config['path']['trained_models'],
                                    '{models2}_trained_top3_species_prediction_{rs}.sav'),
...
rule predict_mouse_snoRNA_label_species_prediction_top3_rs:
    """ Predict the abundance status of all mouse snoRNA based on the
        top3 features (combined_box_hamming. sno_mfe and terminal_stem_mfe). We
        scale the feature_df using the same parameters (mean and stdev) used to
        scale each training set."""
    input:
        ...
        pickled_trained_model = rules.train_models_species_prediction_top3_rs.output.pickled_trained_model
    output:
        predicted_label_df = 'results/tables/mouse_prediction/{models2}_predicted_label_species_prediction_top3_{rs}.tsv',
        scaled_feature_df = 'results/tables/mouse_prediction/{models2}_scaled_features_species_prediction_top3_{rs}.tsv',
        label_df = 'results/tables/mouse_prediction/{models2}_label_species_prediction_top3_{rs}.tsv'
    params:
        random_state = 42
    conda:
        "../envs/python.yaml"
...
rule test_accuracy_species_prediction_top3_rs:
    """ Test model performance on mouse snoRNA data and return their accuracies."""
    input:
        X_test = rules.predict_mouse_snoRNA_label_species_prediction_top3_rs.output.scaled_feature_df,
        y_test = rules.predict_mouse_snoRNA_label_species_prediction_top3_rs.output.label_df,
        pickled_trained_model = rules.train_models_species_prediction_top3_rs.output.pickled_trained_model
    output:
        test_accuracy = os.path.join(config['path']['test_accuracy_mouse'],
                                    '{models2}_test_accuracy_top3_species_prediction_{rs}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/test_models_scale_after_split.py"

rule confusion_matrix_f1_species_prediction_top3_rs:
    ...
    output:
        confusion_matrix = os.path.join(config['path']['confusion_matrix_f1_mouse'],
                        '{models2}_confusion_matrix_w_f1_score_top3_species_prediction_{rs}.tsv'),
        info_df = os.path.join(config['path']['confusion_matrix_f1_mouse'],
                        '{models2}_confusion_matrix_values_per_sno_top3_species_prediction_{rs}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/confusion_matrix_f1_scale_after_split.py"

rule predict_mouse_snoRNA_label_species_prediction_top3_wo_dup_rs:
    """ Predict the abundance status of all mouse snoRNA based on the
        top3 features (combined_box_hamming. sno_mfe, terminal_stem_mfe and
        host_expressed) wo duplicate snoRNAs (snoRNAs with identical feature values).
        We scale the feature_df using the same parameters (mean
    ...
    input:
        ...
        pickled_trained_model = rules.train_models_species_prediction_top3_rs.output.pickled_trained_model
    output:
        predicted_label_df = 'results/tables/mouse_prediction/{models2}_predicted_label_species_prediction_top3_wo_dup_{rs}.tsv',
        scaled_feature_df = 'results/tables/mouse_prediction/{models2}_scaled_features_species_prediction_top3_wo_dup_{rs}.tsv',
        label_df = 'results/tables/mouse_prediction/{models2}_label_species_prediction_top3_wo_dup_{rs}.tsv'
    params:
        random_state = 42
    conda:
        "../envs/python.yaml"
...
rule test_accuracy_species_prediction_top3_wo_dup_rs:
    """ Test model performance on mouse snoRNA data and return their accuracies.
        ...
        redundancy."""
    input:
        X_test = rules.predict_mouse_snoRNA_label_species_prediction_top3_wo_dup_rs.output.scaled_feature_df,
        y_test = rules.predict_mouse_snoRNA_label_species_prediction_top3_wo_dup_rs.output.label_df,
        pickled_trained_model = rules.train_models_species_prediction_top3_rs.output.pickled_trained_model
    output:
        test_accuracy = os.path.join(config['path']['test_accuracy_mouse'],
...
rule confusion_matrix_f1_species_prediction_top3_wo_dup_rs:
    """ Create a confusion matrix (all True/False positives or negatives) for
        each model in order to compare whether they misclassify the same snoRNAs
        or not. Compute also the F1 score per model. Do this on mouse snoRNAs."""
    input:
    ...
    output:
        confusion_matrix = os.path.join(config['path']['confusion_matrix_f1_mouse'],
                        '{models2}_confusion_matrix_w_f1_score_top3_species_prediction_wo_dup_{rs}.tsv'),
        info_df = os.path.join(config['path']['confusion_matrix_f1_mouse'],
                        '{models2}_confusion_matrix_values_per_sno_top3_species_prediction_wo_dup_{rs}.tsv')
    conda:
        "../envs/python.yaml"
    script:
        "../scripts/python/confusion_matrix_f1_scale_after_split.py"
